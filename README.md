...
### WPF DB바인딩 연습 - [소스](./day01/Day01Wpf/WpfBasicApp01/MainWindow.xaml), [소스](./day01/Day01Wpf/WpfBasicApp01/MainWindow.xaml.cs)
1. 프로젝트 생성
...
6. DB연결
    ...
    5. DB명령객체(`SqlCommand`) : 쿼리를 컨트롤하는 객체, 생성시 쿼리와 연결객체
        ...
        - ExecuteNonQuery() : INESRT, UPDATE, DELETE 문과 같이 transaction이 발생하는 쿼리실행 사용 메서드 
    6. DB데이터어댑터(`SqlDataAdapter`) : 연결이후 데이터처리를 쉽게 도와주는 객체
        - DB명령객체처럼 쿼리를 직접싱행할 필요없음
...
- MVC : Modeol View Contorller 패턴
    ...
    - Spring Boot, APS.NET, jDango 등 웹개발 아키텍처패턴으로 표준으로 사용 
...
### WPF MVVM 연습
1. 프로젝트 생성 - [소스](./day01/Day01Wpf/WpfBasicApp02/View/MainWindow.xaml) [소스](./day01/Day01Wpf/WpfBasicApp02/ViewModel/MainViewModel.cs)
2. WPF DB바인딩 연습시 사용한 UI 복사
3. Model, View, ViewModel 폴더 생성
4. MainWindow.xaml을 View로 이동
5. App.xaml StrtupUri 수정
6. Model폴더 내 Book클래스 생성
    - INotifyPropertyChanged 인터페이스 : 객체내의 어떠한 속성값이 변경되면 상태를 C#에게 알려주는 기능
    - PropertyChangedEventHandler 이벤트 생성
7. ViewModel폴더 내 MainViweModel클래스 생성
...
9. MainView.xaml 컨트롤에 바인딩 작업
    - 전통적인 C# 방식 : x:Name사용(바하인드 사용필요), 마우스이벤트 추가
...
### MVVM Framework
- MVVM 개발자체가 어려움. 개발시 MVVM 템플릿을 만드는데 시간이 많이 소요. 난이도 있음
- 조금 쉽게 개발하고자 3rd Party에서 개발한 MVVM 프레임워크 사용
- 종류
    - `Prism` : MS계열에서 직접 개발시 사용. 모듈화잘되어 있음. 커뮤니티 활발
        - 진입장벽 높음
...
### CommunityToolki.MVVM 다시 
1. WPF프로젝트 생성
2. 필요 라이브러리 설치
    - CommunityToolkit.MVVM
    - MahApps.Metro
    - MahApps.Metro.IconPacks
3. Models, Views, ViewModels폴더 생성
4. MainWindows.xaml 삭제
5. App.xaml StartupUri도 삭제
6. Views/MainView.xaml 생성
7. ViewModels/MainView.cs 생성
8. App.xaml Startup 이벤트 추가
    - App.xaml.cs 로직 추가
9. App.xaml MahApps.Metro 관룍 리소스 추가
10. MainView에 MetriWindow로 변경
...
4. `NLog.config`를 출력 디렉토리 복사
5. Debug, Trace는 출력이 되지 않는다 
...
5. MainView.xaml, MainViewModel 메인화면 MVVM 작업
    ...
    - ContentControl추가
6. 하위 사용자컨트롤 작업
    - BookGenger(Viewm, ViewModel)
    - Books(Viewm, ViewModel)
7. Models > Genre(DivisionTbl) 모델 작업
8. BokGengerViewModel DB처리 구현
...
### DB연결 CRUD 연습(계속)
1. BookGenre에서 INSERT, UPDATE기능 구현 
2. NLog.config 생성
...
4. 각 ViewModel에 IDialogCoordinator 관련 코딩 추가
    ...
    - View, ViewModel연동 시IDialogCoordinator 연결
...
2. NuGet 패키지 사용할 라이브러리 설치
    ...
    - MySQL.DATA 
    - NLog
3. 폴터생성 : Helpers, Models, Views, ViewModels
4. MVVM 구조 초기작업
5. UI 구현
...
7. 데이터그리드 더블클릭해서 상세정보 표시
    - NuGet 패키지에서 Microsoft.Xaml.Behavirs.Wpf설치
...
https://github.com/user-attachments/assets/9c154174-cddf-4405-8133-c72103eadb08
