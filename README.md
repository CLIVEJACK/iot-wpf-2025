...
1. 그리드 표현 아이템 조정
2. 메인창내용을 구글맵창으로 이동
3. CefSharp.Wpf로 구글 맵지도 표현
4. 위도(Lat, "Latitude"),경도(Lng, "Longitude") 표현
...
<img src="./image/wpf0022.jpg" width="600">

- 전면부

<img src="./image/wpf0023.jpg" width="600">
...
- publish / Subscribe 라는 출판쪽 용어로 사용
    ...
    - Subsribe(구독) : 필요한 메시지를 수신받아서 사용
- Server(MQTT 브로커)/Client 프로그램으로 동작
- 데이터는 휘발성 : 받은 사람이 없으면 데이터는 사라짐. DB에 저장하는 구성을 해줘야 함
...
1. MQTT 브로커 설치
    ...
    - 설치 후 서비스에서 서비스 중지
2. Mosquitto 모스키토 성정파일 수정
    - mosquitto.conf 문서에디터 관리자 모드로 실행
...
3. Models, Views, ViewModels 파일 생성    
4. MainWindow 바인딩 처리
5. MainViewModel에서 바인딩 속성 초기화

    <img src="./image/wpf0025.png" width="600">
...
6. MqttPub.py와 publish된 IoT데이터 WPF에서 Subscribe예제
    ...
    https://github.com/user-attachments/assets/bd7d2f7b-bf35-4d9f-8ee9-c564cb5ecde4
